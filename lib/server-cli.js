...
module.exports = {
    ...
    run: function (args, callback) {
        ...
        this.loadOptions();
        this.cli.parseArgs(args, function (err) {
            if (this.cli.loggedHelp) { return callback(); }
            try {
                var server = this.createServer(this.port(), this._binding.value);
    ...
    loadOptions: function () {
        this._port = this.cli.opt("-p", "--port", "The port to run the server on", {
            defaultValue: 1111,
            validators: { "integer": "--port ($1) must be a number" }
        });

        this._binding = this.cli.opt("-b", "--binding", "The address to bind the server to", {
            hasValue: true
        });
    },
